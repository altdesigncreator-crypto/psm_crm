...
-- Optional evidence photos — captured at Add Lead time, or attached later
-- from the lead's own page if skipped. Nullable, no default: most leads
-- will never touch a site visit or a scheduled appointment at all.
...
-- via its unguessable URL. Uploads are keyed by folder = lead id (not
-- uploader id, unlike avatars/check-ins above) since more than one person
-- can legitimately attach or replace a given lead's photo over its
-- lifetime; can_manage_lead_photos() gates who that is, mirroring the same
-- visibility rule used to monitor/edit the lead itself.
insert into storage.buckets (id, name, public)
