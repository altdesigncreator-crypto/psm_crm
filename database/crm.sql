...
create policy profiles_select on public.profiles for select
  to authenticated using (
    ...
    or (public.current_role() = 'manager' and department_code = public.current_department())
  );
    ...
    or exists (
      ...
      where l.id = lead_assignments.lead_id
        and (l.owner_id = auth.uid()
             or (public.current_role() = 'manager' and l.department_code = public.current_department()))
    )
  );
...
begin
  ...
  if not (
    public.is_admin_or_above()
    or (public.current_role() = 'manager' and v_dept = public.current_department())
  ) then
  ...
  to authenticated using (
    ...
    or exists (
      select 1 from public.leads l where l.id = follow_ups.lead_id
        and ((public.current_role() = 'manager' and l.department_code = public.current_department())
...
create policy followups_insert on public.follow_ups for insert
  to authenticated with check (
    public.is_admin_or_above() or public.owns_lead(lead_id)
  );

-- Managers are deliberately excluded from insert/update (FRD: Follow-up =
-- "View Only" for Manager) — only Admin/Boss/Super-Admin or the owning
-- salesperson may add/edit follow-up records.
...
create policy appointments_write on public.appointments for all
  to authenticated
  using (public.is_admin_or_above() or public.owns_lead(lead_id))
  with check (public.is_admin_or_above() or public.owns_lead(lead_id));
