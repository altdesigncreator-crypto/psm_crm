          ...
          <Card key={c.id} className="shadow-card hover:shadow-card-hover transition-all duration-300 rounded-xl border-0 overflow-hidden flex flex-col h-full group">
            ...
            <CardContent className="p-4 flex flex-col flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <NameLink id={c.employee_id} name={nameOf(c.employee_id)} showAvatar={false} className="text-xs font-semibold" />
              </div>
              ...
              <div className="bg-black/80 text-white px-5 py-3 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-white/70 mt-0.5">
                    <Link to={`/profile/${lightboxPhoto.employee_id}`} className="hover:underline underline-offset-2 text-white/90">{nameOf(lightboxPhoto.employee_id)}</Link> · {formatTime(lightboxPhoto.check_in_time)}
                  </p>
                </div>
