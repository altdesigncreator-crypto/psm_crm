    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="sm" className="gap-1.5 text-xs font-medium text-primary hover:bg-primary/10">
          ...
          {t('statusColor.customizeColors')}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-balance">{t('statusColor.pipelineStageColors')}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-2 max-h-[60vh] overflow-y-auto pr-1">
          {LEAD_STAGES.map((stage) => {
            const label = enumLabel('stage', stage.value, stage.label, lang);
            return (
            <div key={stage.value} className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  aria-label={`${t('statusColor.colorForPrefix')} ${label}`}
                />
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button size="sm" onClick={handleSave} className="bg-primary text-primary-foreground">{t('statusColor.saveColors')}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
