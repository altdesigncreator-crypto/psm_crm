...
import { getDepartment, filterVisibleLeads } from '@/lib/roleUtils';
...
export default function PipelineBoard() {
  ...
  const [leads, loading] = useOfflineCollection<Lead>(
    'leads',
    role,
    getDepartment(role || ''),
    user?.email,
        ...
        {columns.map((col) => (
          <div key={col.status} className="flex flex-col gap-3">
            ...
            <div
              ...
              style={{
                borderColor: `${statusColors[col.status]}40`,
                backgroundColor: `${statusColors[col.status]}10`,
              }}
            >
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-sm font-semibold" style={{ color: statusColors[col.status] }}>
                  {col.label}
                </span>
              </div>
            ...
            <div className="flex flex-col gap-2.5">
              {col.leads.map((lead) => (
                <Card
                  key={lead.id}
                  className="shadow-sm rounded-xl border border-border/60 hover:border-primary/30 hover:shadow-card transition-all duration-200 cursor-pointer active:scale-[0.98]"
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {canMoveForward(lead.status) && (
                        <button
                          ...
                          style={{
                            borderColor: `${statusColors[getNextStatus(lead.status)]}40`,
                            color: statusColors[getNextStatus(lead.status)],
                            backgroundColor: `${statusColors[getNextStatus(lead.status)]}08`,
                          }}
                        >
        ...
        <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-sm">
          ...
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">
              <span className="font-medium text-foreground">{selectedLead?.name}</span> ကို မည့် stage သို့ ရွှေ့မည်လဲ။
            </p>
            <Select
