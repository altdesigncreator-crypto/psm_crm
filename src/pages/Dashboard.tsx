...
export default function Dashboard() {
  ...
  const agentPerformance = useMemo(() => {
    ...
    return sorted.map(([id, { name, count }]) => ({ id, name, count, pct: Math.round((count / max) * 100) }));
  }, [soldLeads, nameOf]);
                  ...
                  <div key={agent.id} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <NameLink id={agent.id} name={agent.name} showAvatar={false} />
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-xs font-semibold text-primary tabular-nums">{agent.count} sold</span>
                    ...
                    <div key={lead.id} className={`flex gap-4 py-3 ${!isLast ? 'border-b border-border' : ''}`}>
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-foreground leading-snug">
                          {lead.owner_id ? <NameLink id={lead.owner_id} name={nameOf(lead.owner_id)} showAvatar={false} /> : 'Someone'} added a new lead for <span className="text-primary">{lead.name}</span>
                        </p>
